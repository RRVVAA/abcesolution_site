        ...
        <div class="conteudo">
            <div class="rows">
                <div class="col-12 m-auto text-center">
                    <span class="d-block text-center h4 text-escuro mt-1">Escolha um <span
                                class="text-azul">plano ideal</span> para sua empresa</span>
                ...
                @foreach($planos as $plano)
                    ...
                    <div class="col d-flex mb-3">
                        <div class="caixa {{($plano->plano->destaque=='S') ? 'plano3' : 'plano1'}}">
                            ...
                            <p class="h5"><span class="text-escuro">Por apenas</span> <strong
                                        class="d-block  h4">R$ {{$plano->preco}}</strong></p>+
                            setup {{$plano->plano->valor_setup}}</p>
                            <p class=""><span class="text-escuro">Total: </span> <strong class="d-block  h4"><small
                                            style="font-weight: 100">R$</small> {{ formataNumeroBr($total) }}</strong>
                            </p>
                            <ul>
                                <li><i class="fas fa-check"></i> {{$usuario}}            </li>
                                <li>
                                    <i class="fas fa-check"></i> {{ ($plano->limite_nfe == -1) ? " Nfe Ilimitado " : $nota }}
                                </li>
                            ...
                            <a href="{{route('cadastro',$plano->id )}}"
                               class="btn btn-verde d-table m-auto scroll-page"><i class="fas fa-arrow-right"></i> Quero
                                testar</a>
                        </div>
                    </div>
                @endforeach
